        ...
        <div className="pt-24 pb-16 container mx-auto px-4 animate-fade-in">
            ...
            <div className="bg-white rounded-lg shadow-xl overflow-hidden">
                ...
                <div className="p-8 md:p-12">
                    <p className="text-base font-semibold text-indigo-600 mb-2">{project.category}</p>
                    ...
                    <div className="mt-8"><h2 className="text-2xl font-bold text-gray-800 mb-4">Contractors</h2><div className="flex flex-wrap gap-4">{project.contractors.map(c => (<div key={c.id} className="flex items-center space-x-3 bg-gray-100 p-2 rounded-lg"><img src={c.image} alt={c.name} className="w-12 h-12 rounded-full object-cover" /><span className="font-semibold text-gray-700">{c.name}</span></div>))}</div></div>
                    <div className="mt-8"><h2 className="text-2xl font-bold text-gray-800 mb-4">About</h2><p className="text-gray-700 leading-relaxed">{project.details.summary}</p></div>
                </div>
            </div>
            <div className="mt-12"><h2 className="text-3xl font-bold text-gray-800 mb-10 text-center">Project Timeline</h2><Timeline data={project.timeline} /></div>
                ...
                >
                    {project.gallery.map((item, index) => (
                        <DraggableCardBody key={index} className={item.className + " "}>
                            <img src={item.image} alt={`Gallery ${index}`} className="pointer-events-none relative z-10 h-full w-full object-cover rounded-md shadow-lg" />
